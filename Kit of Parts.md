...
A Kit of Parts is the form in which a [[Prototype]] is maintained so that it can serve everyone without forcing anyone.

Because no two domains are alike — no two [[Individuals]], [[Group|Groups]], or [[Community|Communities]] share the same conditions, capacities, or needs — a Prototype delivered as a single rigid whole would fit almost no one. So [[The Prototype]] is maintained instead as a set of interoperable, fractal Parts that can be selected, combined, and sequenced in many ways to serve each unique [[Instantiation]] ([[Interoperability]]).

Three properties make a Kit of Parts alive rather than a parts bin:
