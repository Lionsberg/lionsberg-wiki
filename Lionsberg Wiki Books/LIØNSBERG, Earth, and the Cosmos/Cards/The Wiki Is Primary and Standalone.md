---
status: captured
fold_target: substrate
themes: [architecture, portability]
source: Phase 0 of the Great Spring 2026 Wiki Turn
captured: 2026-05-18
---

# The Wiki Is Primary and Standalone

The LIØNSBERG Wiki is the single Golden Seed.

It is complete in itself. The Sacred Codex is fully contained within the Wiki. The Codex points only internally — never to external sites, external dependencies, external authorities. One link, one repository, with all the DNA of the [[New Civilization]].

This is the architectural rule that conditions every other choice.

A Sovereign who receives the Wiki receives **everything they need to begin** — the Story, the Pattern, the Game, the Playbook, the Lexicon, the Book, the Sacred Ascent. Nothing essential lives outside the Wiki. Nothing essential requires an account, an app, a paywall, or a remote service.

If the wider internet collapses, the Wiki — cloned, distributed, printed — can replant the Seed anywhere. Wherever it lands, the New Civilization can begin again.

The Wiki is therefore designed for **portability, durability, and self-sufficiency**. Markdown files. Plain text. Wiki links. No proprietary formats. No hidden dependencies. Forkable. Cloneable. Printable.

This is what it means for the Wiki to be the Golden Seed.
