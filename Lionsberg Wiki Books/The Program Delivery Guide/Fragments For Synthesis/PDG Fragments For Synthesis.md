...
7 fragment files that were previously orphaned at the wiki root. They appear to be from an **earlier Program Management / Program Delivery sketch** distinct from the Playbook fragments:
...
Obsidian resolves wiki links by filename, so any existing `[[3.1.5 PM Backbone]]` / `[[PM Navigation Meeting]]` / etc. links continue to resolve correctly from this location.
